...
int maximoBeneficioBasico(vector<accion>& acciones, int presupuesto, int i, vector<bool> &accionesCompradas) {
    ...
    if(acciones[i].getPrecio() <= presupuesto){                                 //Me permite comprar la accion 
        accionesCompradas[i] = true;    
        beneficio = acciones[i].getBeneficio() + maximoBeneficioBasico(acciones, presupuesto - acciones[i].getPrecio(), i + 1, accionesCompradas);   //seguir comprando acciones si queda presupuesto 
    }

    int beneficio_aux = maximoBeneficioBasico(acciones, presupuesto, i + 1, accionesCompradas); 
...
int main(int argc, char *argv[]) {
    ...
    for (int argumento = 1; argumento<argc; argumento++) {
        ...
        vector<bool> accionesCompradas(acciones.size(), false); 
        int max_beneficio = maximoBeneficioBasico(acciones, dineroMax, 0, accionesCompradas);
        tf= std::chrono::high_resolution_clock::now(); // Cogemos el tiempo en que finaliza la ejecuciÛn del algoritmo

        cout << "Maximo beneficio conseguido: " << max_beneficio << endl; 

        cout << "Acciones compradas: " << endl; 
        for(int i = 0; i < accionesCompradas.size(); i++){
            if(accionesCompradas[i]){
                cout<<acciones[i].getEmpresa()<<" (P:"<<acciones[i].getPrecio()<<" B:"<<acciones[i].getBeneficio()<<"); ";
            }
        }
        cout << endl; 
